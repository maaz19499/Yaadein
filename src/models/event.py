...
class Event(Base):
    ...
    host_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
    slug: Mapped[str] = mapped_column(String, unique=True, nullable=False)
    face_search_enabled: Mapped[bool] = mapped_column(Boolean, default=False)
    plan: Mapped[str | None] = mapped_column(String, nullable=True)  # CHECK plan IN ('basic', 'premium', 'professional')
    storage_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
