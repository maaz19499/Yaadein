...
class Media(Base):
    ...
    status: Mapped[str | None] = mapped_column(
        String, nullable=True
    )  # CHECK status IN ('pending_verify', 'scanning', 'processing', 'visible', 'rejected', 'duplicate')
    file_size_bytes: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
    mime_type: Mapped[str | None] = mapped_column(String, nullable=True)
    checksum: Mapped[str | None] = mapped_column(String, nullable=True)
    phash: Mapped[str | None] = mapped_column(
        BIT(64), nullable=True
    )  # Stored as BIT(64), handled as 64-char binary string
...
class Export(Base):
    ...
    scope: Mapped[str | None] = mapped_column(
        String, nullable=True
    )  # CHECK scope IN ('single', 'album', 'full_event')
    status: Mapped[str | None] = mapped_column(
        String, nullable=True
