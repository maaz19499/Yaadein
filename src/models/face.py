...
class FaceConsent(Base):
    ...
    purge_executed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
...
class FaceEmbedding(Base):
    ...
    uploader_consent_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("face_consents.id", ondelete="CASCADE"), nullable=False
    )
